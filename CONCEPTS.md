...
### needs_manual
The FMV verdict emitted when even the fallbacks can't defensibly price a book (raw sold comps too thin, target grade's bucket empty and interpolation unsupported). It is a deliberate **punt to a human/LLM**, not a failure — the book becomes a [[Hand-Priced Row]], priced with judgment inside the `/comic:fmv` skill rather than auto-bid on a shaky estimate. Automating away a `needs_manual` on a high-value key removes the human check exactly where a mistake costs the most.
...
### Active-Ask Ceiling
The lowest price at which the same book, at the same grade and (for a slab) the same certifier and [[Label]], is currently listed for sale, attached to a row whose price was withheld as [[needs_manual]]. It bounds what buying the book outright would cost today; it is not evidence of what the book sells for, so it never enters the band or the bid cap and never softens the refusal. It only tells the human deciding the row what a live ask looks like.

A row carrying no ceiling is a legitimate outcome (no live asks), which makes the absence of one shape-identical to the lookup never having run, the same trap a [[Fetch Error]] names for sold comps. A ceiling lookup that errors must therefore be told apart from one that found nothing, and the only proof the lookup fires at all is a live run against real listings.
...
### Multi-Book Lot
A single sold listing that prices two or more graded books together — two issues, two grades, one price. It is neither a comp of any one of those books nor a rung of any ladder: parsed as a single grade-and-price point it lands above the genuine rungs and inverts a monotonic ladder, so the [[CGC Proxy]]'s second pass refuses the whole ladder rather than one comp. Because the contamination is a property of the listing, not of the target being priced, the exclusion applies on every pass that admits graded comps, certified target or raw; a stored copy in the [[Comps Ledger]] is stamped with an exclusion code rather than deleted. Distinct from a [[Variant Drop]], which excludes a comp for being a different edition of the same book.
...
### Label
The grading company's certification category on a slab: **Universal** (blue), the plain certified grade; **Signature Series** (yellow), a witnessed autograph; **Qualified** (green), a book with a named defect excluded from the grade; **Restored** (purple); **Conserved**, a book that received a lesser, disclosed conservation treatment short of restoration. A sixth value, **other**, is reserved in the vocabulary for a label none of the five parses to, though nothing currently produces it. Each trades in its own market at the same numeric grade, so a label mismatch between a target and a comp is an identity mismatch, not a condition difference. Only Universal is priced; the other four are identified and punt to [[needs_manual]] with the label as the reason (see [[Graded Pricing Mode]]).
...
### Pricing Basis
How an `fmv` band was produced, stored on the row so a cache hit reproduces the same confidence and bid factor: **direct** (priced from sales at the exact grade), **interpolated** (raw [[Grade-Curve Interpolation]]), **ladder** (a slab target read off neighboring certified rungs), **lone sale** (a slab target with exactly one fresh sale at its grade, accepted because certified rungs on both sides bracket it), **proxy** ([[CGC Proxy]]). It exists because the stored confidence label collapses tiers and a notes token fails open on a reword, so neither can carry the reduced bid factor a ladder, lone-sale, or interpolated row needs on the [[Money Path]]. The store enforces the set of bases, so a new basis is a schema change, not a new string.

### Graded Pricing Mode
The automated pricing path for a [[Certified Grade]] target (BUI-930), separate from the raw pipeline rather than an extension of it: it prices from that certifier's own slab sales at the target's [[Label]] and page quality, and deliberately skips every raw-market fallback — grade-curve interpolation, [[CGC Proxy]], the ungraded anchor, first-party comps — because each of those would price a slab off the wrong market. Only a Universal-label CGC or CBCS slab prices this way; any other label or certifier is a distinct market and is [[needs_manual]] before any comp is fetched.

At two or more weighted sales exactly at the target grade it prices directly (`pricing_basis` `direct`); below that the target's own rung is dropped from the ladder and its neighbors interpolate across the gap — a rung holding a single sale is still eligible to anchor that interpolation, since a vintage slab routinely has exactly one recorded sale at a given grade (`pricing_basis` `ladder`, LOW confidence, a 0.60 bid factor). A single fresh sale exactly at the target grade is accepted on its own when certified rungs below and above bracket it and the bracket is not too wide (`pricing_basis` `lone sale`, LOW confidence, a lighter haircut than the ladder's). Stored slab comps age by the calendar from the day of the run, not from the newest comp, so a pool does not stay young because nothing new sold. The [[Comps Ledger]]'s stored slab comps join the live pool for this mode only, weighted down by age. Gated by a run-level read-side probe that refuses every certified target outright when the comics server predates the certifier-aware price identity, so a certified price can never silently land on a raw row's price at the same numeric grade.

### Slab Watch Set
The wish-listed books whose expected value clears a floor, chosen as the set whose certified sales are worth collecting on a schedule rather than only when a listing is being priced. A scheduled collection run fetches each book's slab sales into the [[Comps Ledger]] so that [[Graded Pricing Mode]] finds a deeper ladder when a slab of that book finally appears. Membership is a per-book override on top of the value floor, and a book collected recently is skipped. The run reports success only when every book fetched and stored; a single failed book withholds the success signal, because a partial run that reported success would hide exactly the books whose ladders stayed thin.
...
### Hand-Priced Row
An FMV row whose band a human set by judgment rather than the comp pool computing it — typically the resolution of a [[needs_manual]], or an operator's deliberate rejection of a pooled answer that priced the wrong market. It is the one kind of FMV row the pricing pipeline treats as **authoritative over its own output**: a default batch refresh must leave it untouched however stale it looks, because staleness is not evidence against a human's judgment the way it is against a machine's.

Overwriting one is permitted but never silent: only an explicit force run may replace a hand-priced band, and it must echo the old notes before doing so, since the reasoning is the part that cannot be recomputed. Two rules make that protection real rather than nominal. **Provenance is claimed, not inferred** — the row is recognized as hand-priced because the operator marked it as such when writing it, so an unmarked hand edit is indistinguishable from a machine row and forfeits the protection entirely. The claim lives in a **column** (`fmv.provenance`), not in the prose beside it: a claim kept as a prefix in a freetext notes field is lost to any reword, and the loss is silent and in the expensive direction. A notes prefix is still honoured as a fallback for rows written before the column existed, but writing one is no longer how a claim is made. And **the protection must resolve the row by the same identity the write uses**: a guard that looks a row up one way while the upsert reaches it another will return a confident verdict about a row the write never touches. Where provenance cannot be determined at all — an unreachable lookup, or an identity matching several candidate rows — the row is skipped rather than recomputed: *don't know* is treated as *might be hand-priced*, because the two failure directions cost wildly different amounts (a wrongly-skipped machine row is a stale price; a wrongly-recomputed hand row is a destroyed judgment on a book someone chose to price by hand precisely because it was valuable).
