...
def create_parser() -> argparse.ArgumentParser:
    ...
    p_authcheck = coll_sub.add_parser(
        ...
        epilog=(
            ...
            "whose release dates disagree — the blast radius, whether or not "
            "the entry itself is what puts the rows together). BUI-671 splits "
            "that same list into cross_volume_ambiguities_newly_created (only "
            "the entry creates it) and cross_volume_ambiguities_already_present "
            "(already live today via the base normalizer or a shipped entry) — "
            "read the split before deciding an entry is worth merging. "
