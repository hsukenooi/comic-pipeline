"""Hatch build hook (BUI-305): stamp the wheel with the git SHA/date it was
built from.

Verified this resolves the real repo HEAD under `uv tool install
"$REPO_ROOT/apps/fmv"` (what scripts/install.sh runs): that local-path install
...
class GitStampBuildHook(BuildHookInterface):
    ...
    def initialize(self, version: str, build_data: dict) -> None:
        ...
        stamp_dir = Path(tempfile.mkdtemp(prefix="fmv-build-stamp-"))
        stamp_file = stamp_dir / "_fmv_build_stamp.py"
        stamp_file.write_text(f"GIT_SHA = {sha!r}\nGIT_DATE = {date!r}\n")

        build_data.setdefault("force_include", {})[str(stamp_file)] = "_fmv_build_stamp.py"
