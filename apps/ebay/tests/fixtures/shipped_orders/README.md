...
| `merchant_unknown_carrier.json` | a labelled `QXPRESS…` number | a real tracking number for a carrier with no rule here (an invented one, so it can never accidentally start matching) is reported as `unknown_carrier`, never emitted unvalidated |
...
## BUI-967: Shopee SPX, and the shipping-subject false positive

| file | shape | what it proves |
| --- | --- | --- |
| `merchant_shopee_labeled.json` | a labelled Shopee Xpress `SPXSG…` number | previously reported `unknown_carrier` (see the old `merchant_unknown_carrier.json`, since repointed at an invented carrier so this case could be told apart from it); now recognized shape-only via the label path — `SPX` + a 2-letter country code + digits, unverified because no check-digit algorithm is documented and no real SPX number has appeared in this account's mail |
