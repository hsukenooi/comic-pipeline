...
# ─── Grade parsing ────────────────────────────────────────────────────────────
#
# `_NUMERIC_GRADE_RE` and `_LETTER_PATTERNS` moved to grade_tokens.py (BUI-923)
# — identify's certifier-aware slab parsing needs the same numeric-grade
# lookarounds and letter-grade table, and sold_comps already imports from
# ebay_fetch, so grade_tokens is the shared third module both import from
# (see its docstring). Imported above; parse_grade's own logic is unchanged.
