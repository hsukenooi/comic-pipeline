...
# every route declared on `router` below — including ones added years from now
# — is covered with zero per-endpoint code. That is BUI-601's acceptance
# criterion, and it is why the ticket's stale hand-enumeration of "four
# un-persisted 422 sites" (there are 16 in this file today) is an argument FOR
# the generic mechanism rather than a list to work through.
...
router = APIRouter(route_class=LedgerRoute)
...
# ---------------------------------------------------------------------------
# Silent-failure observability (BUI-601 + BUI-602)
#
# Two halves of one idea. The ledger answers "what did the server refuse to
# store?" — the BUI-593 shape, where a write 422'd and the book ended up
# nowhere. The heartbeats answer "did the job that should have written it even
# run?" — the fails-green shape, where a dead job and a healthy one that found
# nothing look identical. Neither question is answerable from error logs alone,
# which is why both needed a table.
# ---------------------------------------------------------------------------


@router.get("/api/comics/health/rejections")
async def api_health_rejections(
    request: Request,
    hours: float = DEFAULT_REJECTIONS_WINDOW_HOURS,
    limit: int = 50,
):
    """Rejected mutating writes in the last `hours` (BUI-601).

    Read-only, and deliberately a plain GET with no auth of its own — it is
    served on the same LAN-local surface as the rest of the dashboard. The
    dashboard badge polls this; a human passing `?hours=720` gets the full
    retention window.
    """
    if hours <= 0:
        raise HTTPException(status_code=422, detail="hours must be > 0")
    db = request.app.state.db
    return rejected_writes_report(db, hours=hours, limit=limit)


@router.get("/api/comics/health/heartbeats")
async def api_health_heartbeats(request: Request):
    """The cadence watchdog verdict for every job in the contract (BUI-602).

    Lives under `/api/comics/health/*` alongside the rejections report because
    both are the dashboard's health surface, while the *ping* below is at
...
@router.post("/api/heartbeat/{job}")
async def api_heartbeat(job: str, request: Request, detail: str | None = None):
    """Record a SUCCESSFUL run of `job` (BUI-602).

    The job name must be one the contract table declares. An unknown name is a
    404, not a silent accept: a typo'd ping that stored fine would leave the
    real job looking dead forever while the watchdog stayed green — the exact
    failure this endpoint exists to prevent.

    Callers must only ping on success as their contract entry defines it.
    Pinging at the *start* of a run, or on a partial/aborted one, converts
    this from a health signal into noise that always reads healthy.
    """
    contract = JOB_CONTRACTS.get(job)
    if contract is None:
        raise HTTPException(
            status_code=404,
            detail=(
                f"Unknown job {job!r}. Declare it in gixen_overlay.db.JOB_CONTRACTS "
                f"(with a cadence and a success definition) first. "
                f"Known jobs: {', '.join(sorted(JOB_CONTRACTS))}"
            ),
        )
    async with _write_locked():
        with write_transaction(_get_db_path()) as wconn:
            row = record_heartbeat(wconn, job, detail=detail)
    return {
        "status": "ok",
        "job": row["job"],
