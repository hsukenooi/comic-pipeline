...
Run all items in a single call with the installed `ebay-fetch` console script (it also
accepts full URLs):
...
**Grade logic:** `ebay-fetch` already checks item specifics, then the title, then the
