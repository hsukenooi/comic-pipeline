...
- `--force`: bypass both the SerpApi cache and the DB cache and recompute everything. **It cannot clear a `one_sided`/`too_wide` flag** — it refetches the *same* market and re-flags identically. To move a flagged book, change the input (`title`/`publisher`/`year`) or widen `--grade-window`; a bare `--force` retry is a wasted no-op. **Hand-priced rows (BUI-533, widened BUI-759):** a row whose `fmv_notes` *opens with* the word `hand` or `manual` in any capitalization (`hand §`, `hand OVERRIDE`, `manual:`, `Manual:` — an operator's manual override, e.g. Batman #251) is skipped entirely by a default run — reported as `skipped N hand-priced row(s) (use --force to overwrite)` and a `skipped_hand_priced` source on that row — even when it's stale enough that normal cache logic would otherwise recompute it. Only `--force` overwrites one, and it echoes the old hand notes to stderr first so they aren't silently lost.
