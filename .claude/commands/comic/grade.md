...
comic-2: Fantastic Four #29 (1964) Marvel — 6 images — current bid $4.25 (2 bids) — tier: not-cheap (estimated: vintage 1964, 4d left)
```

The printed **tier** (`cheap` / `not-cheap`) is the **value signal for the Step 2 value gate** — record it alongside the item id from the printed line so escalation can branch on listing value without re-deriving anything. Read the token right after `tier: ` and **ignore any trailing `(...)`** — that parenthetical is the reason, not part of the verdict.
