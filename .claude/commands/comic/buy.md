...
A non-zero exit code means at least one row did not land (failed or was left not-attempted after a mid-batch halt) — **read the JSON, don't treat non-zero as total failure.** Present the human table the CLI printed (it now includes comic titles, BUI-506) and call out any `failed`/`not_attempted` rows explicitly.
...
**Policy advisories (BUI-621):** `summary.advisories` is a total count and each row's `advisories` array carries the full KTD4 detail (`{code, severity, message, data}`) for that row — non-fatal server-side checks run before the write, never blocking. A batch whose rows all landed can still carry `summary.advisories > 0`; the human table the CLI printed already marks the affected rows and prints an end-of-run count. Handle every advisory the same way `snipe-add.md` § Handling Policy Advisories describes before telling the user the batch is done: read the message, then `gixen edit`/`gixen remove`/acknowledge as the situation calls for.
