...
A non-zero exit code means at least one row did not land (failed, blocked by a policy check per BUI-623, or left not-attempted after a mid-batch halt) — **read the JSON, don't treat non-zero as total failure.** Present the human table the CLI printed (it now includes comic titles, BUI-506) and call out any `failed`/`blocked`/`not_attempted` rows explicitly.

**Output:** the add-batch JSON (also at `--json-out`): `{"summary": {..., "advisories": N}, "halted": bool, "verify_error": ..., "rows": [{"item_id", "status", "max_bid", "grade", "created", "link_attempted", "link_ok", "error", "link_error", "verify", "advisories"}, ...]}`. `error` is set only when the row itself failed to add (`status: "failed"`); `link_error` is a separate field set when the add landed but the FMV link call failed (`status` stays `"added"`/`"updated"`) — don't conflate the two when scanning for failures.
