...
**Output:** Human FMV table to stdout, followed by one compact JSON line per row (`--brief`, BUI-362; `fmv_low`/`fmv_high`/`fmv_notes` added BUI-505; `source` added BUI-549; `certifier`/`label`/`pricing_basis` added BUI-930): `{item_id, comic_id, fmv_id, max_bid, fmv_low, fmv_high, fmv_notes, flag_reason, confidence, source, certifier, label, pricing_basis}`. `certifier`/`label`/`pricing_basis` are `null` on a raw row (unchanged shape for every existing consumer) and carry the price identity (`certifier`, `label`) plus how the band was produced (`pricing_basis`: `direct`|`interpolated`|`ladder`|`proxy`) on a priced row — thread `certifier`/`label`/`cert_number` into `gixen build-batch` at Step 5 the same way `comic_id`/`fmv_id` already are. (Any skip-count summary lines print to stderr, not stdout — BUI-549 — so stdout under `--brief` is pure JSON Lines.) The full structured JSON still lands at `--out`, but **do not read the `--out` file into context** — it's dominated by `queries_used`/`trimmed_pool` (~6k tokens for 7 rows). The brief lines carry everything Steps 4–5 need; keep `--out` on disk for deep dives only (e.g. inspecting the comp pool when CV >100%).
...
A **BIN certified** row (`listing_type: "BIN"` on a certified book, R34/BUI-930) has a real `comic_id` and a real band — unlike the ledger-advisory row above — but `max_bid` is still `null`, deliberately: BIN listings have no auction to snipe. Present the FMV range and confidence, `—` in Max Bid, noting `BIN — buy by hand`. Never propose a max bid for it and never carry it into Step 5.
