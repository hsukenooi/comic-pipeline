...
**Certified (slab) rows (BUI-923):** the identify table's `Cert`/`PQ` columns (identify.md) mean the working list also carries `certifier` (`cgc`/`cbcs`/`other`), the certified numeric `grade`, `cert_number`, `label`, `page_quality`, and `listing_type` for any row with `grade_source: certified` — carry these straight through, don't re-derive them. A certified grade is sealed by the grading company, so it carries no grade risk: Step 2.5 skips these rows outright.
...
Step 1 **Year column verbatim** — blank stays blank, never backfill it
(collection-check.md § Input shape owns the BUI-316/BUI-129 forwarding rule) —
and `variant` when present), then:
...
**at most one copy**, keep every copy (don't drop later-ending ones), let each
still flow through grade/FMV individually (per-copy max bids may differ by
grade), and mark them a bid-group candidate to apply as a shared `"group": N` at
Step 5 (omit it if the user genuinely wants multiple copies). The rules for safe
grouping are owned by `snipe-add.md` § Bid groups — this step only surfaces the
candidates.
...
**Run after the collection check** — only grade comics that survived it (no point assessing condition on a comic the user is skipping). **Skip every certified row outright** (`certifier` set to `cgc`/`cbcs`/`other`, BUI-923) — a CGC/CBCS grade is sealed by the grading company, carries no grade risk, and photo-grading it has nothing to add. Inspect the *remaining* working list for comics where `grade_source` is `"missing"` AND no grade signal appeared in the title or description.
...
**If all comics already have a stated grade:** skip grading by default. **Optionally offer to "grade anyway"** for a stated-grade listing — recommended when the Step 1 advisory flagged the seller as an over-grader, or for a high-value book — since photo-grading a stated-grade listing is the only way the seller-deviation signal (BUI-78) accumulates for over-graders (who almost always state a grade). Keep it user-gated, not automatic. **Never offer "grade anyway" for a certified row** — there is no seller-deviation signal to measure against a sealed CGC/CBCS grade, and no grade risk to reduce.
...
**Certified (slab) rows (BUI-930)** carry the same `item_id`/`title`/`issue`/`year`/`grade` plus `certifier`, `label`, `page_quality`, `cert_number`, and `listing_type` — pass Step 1's values straight through. **Drop `grade_confidence` for these rows**; `comic-fmv` ignores it (a certified grade is not a photo judgement), so sending one is a no-op, not a haircut. A certified row never touches the raw market: no raw pool, no CGC-proxy rescue, no cross check, no ungraded anchor, no first-party comps, and none of §7a's discount factors — it prices from that certifier's own slab sales at that label only.

**Output:** Human FMV table to stdout, followed by one compact JSON line per row (`--brief`, BUI-362; `fmv_low`/`fmv_high`/`fmv_notes` added BUI-505; `source` added BUI-549; `certifier`/`label`/`pricing_basis` added BUI-930; `provenance` added BUI-949; `active_ask_low`/`active_ask_n` added BUI-954): `{item_id, comic_id, fmv_id, max_bid, fmv_low, fmv_high, fmv_notes, flag_reason, confidence, source, certifier, label, pricing_basis, provenance, active_ask_low, active_ask_n}`. `active_ask_low`/`active_ask_n` are `null` except on a needs-manual row where an active Buy It Now search found a matching listing — read it as a **ceiling** (the lowest current ask for the same identity), never as a price to propose; it never sets `fmv_low`/`fmv_high`/`max_bid` and is absent whenever the row priced normally. `certifier`/`label`/`pricing_basis` are `null` on a raw row (unchanged shape for every existing consumer) and carry the price identity (`certifier`, `label`) plus how the band was produced (`pricing_basis`: `direct`|`lone_sale`|`interpolated`|`ladder`|`proxy`) on a priced row — thread `certifier`/`label`/`cert_number` into `gixen build-batch` at Step 5 the same way `comic_id`/`fmv_id` already are. `provenance` (BUI-949) is never `null` — a short "what the price rests on" string built straight from the structured fields (never parsed out of `fmv_notes`): `exact n2`/`ladder 4 rungs` (+ `pq widened`/`ledger-drop N`) on a priced certified row, `direct n7`/`interpolated n5`/`proxy n3` (+ `haircut grade`/`haircut fmv`/`haircut both` below the standard 0.80 factor) on a priced raw row, `refused <reason>` or the bare `<reason>` on a needs-manual row, `n/a` or a skip token otherwise — carry it into Step 4's table and present it at both decision points: the BIN stop, and the max-bid approval gate. (Any skip-count summary lines print to stderr, not stdout — BUI-549 — so stdout under `--brief` is pure JSON Lines.) The full structured JSON still lands at `--out`, but **do not read the `--out` file into context** — it's dominated by `queries_used`/`trimmed_pool` (~6k tokens for 7 rows). The brief lines carry everything Steps 4–5 need; keep `--out` on disk for deep dives only (e.g. inspecting the comp pool when CV >100%).
...
**Needs-manual rows (BUI-86):** a row whose `flag_reason` is set (surfaced directly on its brief line) could not be honestly auto-priced — its `fmv_low`/`fmv_high`/`max_bid` are all `null`. It still has a real `comic_id` (the comic stub was written — this holds for a certified row too, BUI-930: both paths end in the same upsert), so the `comic_id: null` check above will **not** catch it. Gate on `flag_reason` instead: surface these rows as **needs-manual** and do not auto-propose a max bid. The user either hand-prices them (via the `fmv.md` interpolation / CGC-proxy methods) or skips them — never bid the absent number. When `active_ask_n` is non-null (BUI-954), also surface it as `asks from $active_ask_low (nactive_ask_n)` — the lowest current Buy It Now ask for this identity, worded and read as a **ceiling** (a seller's ask, not a sold price); it is display-only context for the user's hand-price decision, never a number to feed into it automatically.

Raw reasons: `one_sided`, `too_wide`, `too_sparse`, `variant_dropped`. **Certified (slab) reasons (BUI-930):** `label_signature_series`/`label_qualified`/`label_restored`/`label_conserved` (a non-Universal label trades in its own market — only Universal is auto-priced) and `certifier_other` (only CGC/CBCS are auto-priced) punt before any fetch; `no_certifier_pool` (no slab comps survived the identity/age filters), `ladder_too_thin` (fewer than three usable rungs after dropping the target), `ladder_non_monotone` (the target's two bracketing rungs invert), and `outside_ladder` (the target sits outside the observed rungs — never extrapolated) punt after fetching. `graded_mode_unavailable` (BUI-928) is gone from this client now that the graded FMV mode has shipped (BUI-930) — the server still accepts it, but `comic-fmv` never emits it; don't reference it in new prose.
...
**Schema-mismatch rows (BUI-930):** a row with `source: "skipped_schema_mismatch"` is the same class of system failure, for certified rows only. Before pricing any certified book, `comic-fmv` probes the comics server for a `certifier` key on its `GET /api/comics` rows; when the server predates BUI-924/925 and lacks it, every certified row in the batch is refused with `comic_id`/`fmv_id`/`db_row` all `null` — nothing was fetched or written, because on an old server a slab price would silently land on the raw row at the same grade. Present it the same way as a lookup-error row: a system failure (comics server needs the BUI-924/925 migration), not a pricing verdict — tell the user to deploy the server and re-run.

**BIN certified rows (R34, BUI-930):** a certified row with `listing_type: "BIN"` still gets fetched, priced, and written — the band and comps are real — but `max_bid` is withheld on purpose (same shape as a ledger-advisory row: real band, null cap). Present the band and confidence; there is no offer, no watch, and no snipe for it — the user buys it by hand at or under the band. This is unrelated to the ordinary "flag Buy It Now listings, skip at the Gixen step" rule above; a BIN certified row still gets priced, it just never reaches Step 5.
...
The CLI returns `max_bid = round_clean(bid_factor × fmv_high)` per row. `bid_factor` is `0.80` by default; `docs/conventions/fmv-math-spec.md` §6 owns the haircut logic that lowers it when grade or comp confidence is low (see Step 2.5 for where `grade_confidence` comes from). When a haircut applied, the row carries `bid_haircut=…` in its `fmv_notes` — read it straight off the brief line (BUI-505 emits `fmv_notes` there); no `--out` read needed. **Also present each row's `provenance` (BUI-949)** — the short cell naming what the price rests on (`direct n7`, `ladder 4 rungs`, `haircut grade`, …, see Step 3's Output note for the full token list). This is the field to show at the two moments a user actually acts on this table — deciding a BIN listing by hand, and approving or overriding a proposed max bid — not a substitute for `fmv_notes`, which stays the full audit trail:

```
| # | Comic | Grade | FMV Range | Max Bid | Provenance | Notes |
|---|---|---|---|---|---|---|
| 1 | Amazing Spider-Man #300 | NM | $800–1000 | $800 | direct n12 | |
| 2 | Invincible #1 | NM 9.4 (photo, LOW conf) | $270–320 | $192 | direct n5 haircut grade | grade_confidence=low, bid_haircut=0.60 |
| 3 | Batman #609 | NM | $40–50 | $40 | direct n8 | ⚠️ ends 2026-05-11 |
| 4 | Fantastic Four #63 | NM+ 9.6 | needs-manual | — | one_sided | manual_review=one_sided — hand-price or skip |
| 5 | Batman #227 | CGC 4.5 | needs-manual (exact=$700@2026-09-17, below=4:$1400 n1, above=5.5:$899 n1) | — | refused ladder_non_monotone | manual_review=ladder_non_monotone — hand-price or skip |
```

Clean-number rounding: $5 step below $50, $10 step from $50–$200, $25 step above $200.

A `needs-manual` row (`flag_reason` set) has no CLI-computed max bid. Present it without a proposed number; the user supplies a hand-derived max bid (via the `fmv.md` interpolation / CGC-proxy methods) or skips it. Don't fabricate a max from the absent FMV. **A refused graded (slab) row carries its evidence along with the reason (BUI-940)** — row 5 above: the exact-grade sale with its sold date, and the nearest priced rung below/above the target grade, straight off the brief line's `fmv_notes` (`exact=`/`below=`/`above=` tokens) — present it beside `manual_review=<reason>` so the user can hand-price without reopening the results file. A raw `needs-manual` row, or a graded pre-fetch punt (`label_*`/`certifier_other`) that never fetched comps, carries none of this — present those with the reason alone, same as before. **Either kind may also carry the `active_ask_low`/`active_ask_n` ceiling (BUI-954)** — present it as `asks from $X (nN)` next to the reason when non-null; it's a display-only market signal (the lowest current Buy It Now ask for this identity), never a number to derive a max bid from.

An **advisory (ledger)** row (`source: "ledger-advisory"`, BUI-663) is the same rule with a sharper edge, because unlike a needs-manual row it *does* show a band: the band came from stored comps after the live fetch failed, and `max_bid` is `null` on purpose. Present the range in the FMV column and `—` in Max Bid, noting `LEDGER-ADVISORY — not a live price`. Its `provenance` reads the bare `ledger-advisory` token (BUI-949) — deliberately not a tier/count cell, since there is no live-priced tier behind it. **Do not compute `0.8 × fmv_high` yourself** — the withheld cap is the entire safety property of this row, and a proposed number reinstates exactly the risk it was withheld to avoid. It also has no `comic_id`, so even if the user hand-prices it, Step 5's FMV link will not attach; prefer re-running `comic-fmv` once the providers recover.

A **BIN certified** row (`listing_type: "BIN"` on a certified book, R34/BUI-930) has a real `comic_id` and a real band — unlike the ledger-advisory row above — but `max_bid` is still `null`, deliberately: BIN listings have no auction to snipe. Present the FMV range and confidence, `—` in Max Bid, noting `BIN — buy by hand`. **This is the exact moment the `provenance` cell earns its keep (BUI-949)** — there is no bid cap to lean on, so `exact n2`/`ladder 4 rungs` (plus `pq widened`/`ledger-drop N` when present) is what tells the user whether to trust the band enough to buy by hand. Never propose a max bid for it and never carry it into Step 5.
...
- `working_list.json` — the working list carried forward: one object per
  ...
  `add-batch`'s stale-listing guard has an end time to check). **Certified
  rows (BUI-926/930)** also carry `certifier`, `cert_number`, and `label`
  (from Step 1, threaded straight through) — pass them instead of
  `seller_grade`/`photo_grade`, never alongside: `build_bid_payload`
  hard-refuses to send `seller_grade`/`photo_grade` for a row that carries a
  certifier, since those are raw photo-assessed observations and a certified
  grade has no photo grade to compare against.
...
**Output:** the add-batch JSON (also at `--json-out`): `{"summary": {..., "advisories": N}, "halted": bool, "verify_error": ..., "rows": [{"item_id", "status", "max_bid", "grade", "created", "link_attempted", "link_ok", "error", "link_error", "verify", "advisories", "reconcile"}, ...]}`. `error` is set only when the row itself failed to add (`status: "failed"`); `link_error` is a separate field set when the add landed but the FMV link call failed (`status` stays `"added"`/`"updated"`) — don't conflate the two when scanning for failures.

**Indeterminate rows (BUI-697):** `status: "indeterminate"` means the add call **timed out** and the end-of-batch reconcile against `/api/comics/snipes` still could not confirm it — the write may well have committed. Never report it as failed, and never take a dependent action on it (re-add, `gixen edit`, `gixen remove`, or editing the rows file) before re-reading live state; doing exactly that is what put ASM #61 live at $1.09 instead of $20 on 2026-08-07. `rows[].reconcile` records what the automatic reconcile saw. Follow `snipe-add.md` § Handling an indeterminate row.

**Policy advisories and blocks (BUI-621/BUI-623):** `summary.advisories` is a total count and each row's `advisories` array carries the full KTD4 detail (`{code, severity, message, data}`) for that row — non-fatal server-side checks run before the write. By default a failing check only surfaces as an advisory and the row still lands, but any check whose `POLICY_BLOCK_<CODE>` env var is on (`packages/gixen-cli/CLAUDE.md` § "Policy checks") 409s the write instead: that row reports `status: "blocked"` (🚫 in the human table) rather than `added`/`updated`, `summary.blocked` counts it, and it drives the non-zero exit code called out above — but it does **not** halt the rest of the batch, unlike a mid-batch server failure. A batch whose rows all landed can still carry `summary.advisories > 0`; the human table the CLI printed already marks the affected rows and prints an end-of-run count. Handle every advisory and every block the same way `snipe-add.md` § Handling Policy Advisories describes before telling the user the batch is done: an advisory's write already landed (`gixen edit`/`gixen remove`/acknowledge), but a blocked row never landed at all, so its remediation is instead to lower the bid and re-add, or clear the blocking condition — never `gixen edit`/`gixen remove`, which assume a live snipe a blocked row doesn't have.
...
- **`rows[].verify.verdict != "fully_linked"`** — surface the row in a table plus its `rows[].verify.guidance` string, verbatim. `no_fmv_at_certifier` (BUI-925) is one such verdict: a linked `fmv` row exists at the bid's grade, but only for a *different* certifier/label than the bid carries (e.g. a raw row exists at 9.6 but the bid is a CGC 9.6 slab). Don't read it as `fully_linked` by proximity to the grade — the guidance string names both the requested and the found certifier/label.
