...
| Invoking Step 3's Bash block without an explicit long timeout | It can legitimately run several minutes (BUI-465 pacing); set the tool call's timeout to 600000ms, or use `run_in_background` for a batch above ~46 wins (BUI-472) |
