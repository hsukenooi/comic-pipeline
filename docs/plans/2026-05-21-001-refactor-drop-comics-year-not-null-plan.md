...
- **R10.** Reconciliation never silently merges a reboot. Two surfaces:
  - **R10a (year-set):** If a `upsert_comic(year=Y)` call arrives and there is a NULL-year row plus any yeared row at the same `(title, issue)` with a year other than Y *and* no exact match at Y, the call raises `ReconciliationConflictError` — the situation requires manual disambiguation (the existence of a different-year row is a reboot signal).
  - **R10b (year=None):** If a `upsert_comic(year=None)` call arrives and there are 2+ yeared reboot siblings for `(title, issue)`, the call raises `ReconciliationConflictError` — without a year hint the caller cannot pick the right run (X-Men 1963 vs X-Men 1991).
- **R11.** Multi-issue lot writes never leave orphan rows on conflict. Before writing any comic/fmv/bid_fmvs row for a multi-issue bid, `api_extract_comics` runs `check_reconciliation_conflict` against every issue. If any issue would conflict, the whole bid lands in `skipped` with the conflict reason — none of its issues are written. This avoids the partial-commit gap that arises from `upsert_comic`'s per-call commit semantics.
