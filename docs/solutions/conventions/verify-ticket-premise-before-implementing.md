...
| BUI-933 | `gixen/plugins.py:_invoke_register_db_tables` swallows a raising `register_db_tables` | **No such symbol.** The savepoint-rollback-and-swallow lives in `_invoke_db_tables_isolated`; the described behavior was right, only the name was wrong (a one-line grep in EM context caught it before spawn) | Shipped as (a) re-raise after the loop (PR #531); filed from an audit, not while editing the file |
| BUI-942 | "the slab path skips the year reader" — identify leaves year blank on 7/8 CGC slabs | **There is no slab path.** `confident_cover_year` is one shared strict gate (Publication Year specific corroborated by a parenthesized title year); slab titles fail it more often because the parens hold the cert, not the year | Shipped as a certified-only corroboration rule (PR #533); 6/8 spike slabs yield a year, exactly the six whose titles state one |
| BUI-921 | the CGC cross-check and proxy rescue "never issue the graded-only fetch" — three runs, only the raw query in `queries_used` | **The fetch fired every time.** A two-book lot poisoned the ladder, `monotonicity_violations` refused it, and the pass wrote nothing to `queries_used`/`slab_comps`, so a refused pass was byte-identical to no pass. The ticket's gate-by-gate trace could not have found it | Shipped as guard-the-pass + record-the-pass (PR #538); see `logic-errors/unreported-second-pass-reads-as-never-fired.md` |
