...
- **BUI-333** (shipped, PR #169) — extended the BUI-323 `_retry_request()`/`_atomic_write_json()` consolidation (pattern 4) beyond `ebay_fetch.py` to `sold_comps.py`, `ebay_search_cache.py`, and `seller_scan.py`, and added `.tmp` cleanup to `_atomic_write_json()` itself so a failed write/replace no longer leaves an orphaned `.tmp` file.
- **BUI-322** (shipped, PR #159) — added a batch-level `consecutive_post_refresh_401s` give-up counter to `grade_photos.py:main()`, on top of pattern 2's refresh-on-401 retry: a second straight post-refresh 401 latches `give_up_refreshing` and stops issuing further force-refresh OAuth POSTs for the rest of the batch (each remaining item still gets its `FETCH FAILED` line), so a systemic/revoked-creds failure no longer masquerades as per-item self-healing and hammers the OAuth endpoint once per remaining item.
- **BUI-331** (shipped, PR #168) — changed that give-up counter's reset condition from "since last full success" to "since last confirmed-healthy auth." A new `ListingContentError` (a `RuntimeError` subclass covering unparseable/malformed bodies and image-CDN download failures) marks a failure that still reached a real HTTP-200; `main()` resets `consecutive_post_refresh_401s` on it, since the token demonstrably worked. A non-200 failure (plain 500/404/network exhaustion) is not proof-of-200 and does not reset, so a genuine revoked-creds streak still trips give-up.
