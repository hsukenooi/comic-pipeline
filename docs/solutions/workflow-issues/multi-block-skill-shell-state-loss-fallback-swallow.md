...
- **BUI-375** — a second, independent recurrence of Trap 1 in sibling skills: `collection-check.md` (5 blocks) and `verify.md` (1 block), freshly formalized as EXECUTOR CONTRACTs by BUI-361, sourced `comics-server.sh` in one fenced block and read `$COMICS_SERVER_URL`/`comics_curl` in later, separate blocks with no re-source — the exact shell-state-loss shape this doc names, minus Trap 2 (no `|| echo ""`/fallback-swallow was involved this time). Fixed in PR #204 (commit 9f0902d) by re-sourcing at the top of every block that reads the var, plus adding this doc's prevention-checklist warning inline in both files. Confirms the trap recurs across skill authors and files, not a one-off in `collection-add.md`.
