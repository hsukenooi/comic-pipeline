...
- `docs/solutions/architecture-patterns/vacuous-partition-guards-and-mutable-identity-keys.md` — the second instance of this doc's "a key silently defines what counts as a duplicate" lesson, in locg-cli's collection store rather than the snipe ledger. There the key is built from provider-supplied strings rather than a UNIQUE index, so an upstream relabel changes it and inserts a duplicate; it also carries the converse trap, where the *detector* rather than the key goes blind.
