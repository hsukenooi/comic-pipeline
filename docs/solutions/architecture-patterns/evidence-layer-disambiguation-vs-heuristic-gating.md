...
The lifetime bound that closed the review's P0 (group-number reuse):

```python
def _group_won_before(db, item_id, snipe_group, end_dt, added_at_iso) -> bool:
    ...
...
- Tickets: BUI-50 (incident), BUI-85 (vanish handling), BUI-146 (accepted-risk closure with recorded premise), BUI-363 (bid groups — premise invalidated), BUI-371 (the fix, PR #205). Follow-ups: BUI-381 (group-evidence durability), BUI-382 (fallback write hygiene).
