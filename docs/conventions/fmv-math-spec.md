...
**Page-quality preference.** When at least two comps share the target's `page_quality`, the pool narrows to just those; otherwise every quality is pooled and the row notes the fallback (`page_quality_fallback_reason: "too_few_matches"`). Page quality moves vintage slab prices enough to prefer a same-quality match, but not enough to refuse pricing when one doesn't exist. **The preference must never starve the ladder tier it doesn't apply to (BUI-939).** Scoping is decided once, up front, but which pool actually prices the book is decided per tier: the EXACT tier always reads the scoped pool (a scoped pool that prices directly never widens — the whole reason to scope is choosing that pool over the wider one), and only when the scoped pool would go to the LADDER tier instead is a second, independent check run — count the scoped pool's anchor-eligible rungs (effective n ≥ `GRADED_LADDER_MIN_BUCKET_N`, excluding the target grade) the same way the ladder tier itself counts them below. Fewer than `GRADED_LADDER_MIN_RUNGS` remaining and the ladder widens to the whole pool (`page_quality_fallback: true`, `page_quality_fallback_reason: "ladder_starved"`) before pricing; a scoped pool with enough rungs prices from page-quality-matched comps only, exactly as before. This can only add comps back in — it never touches the exact-tier decision, and a widened ladder can still refuse `ladder_too_thin` if the whole pool doesn't have enough rungs either.
...
- `ladder_too_thin` — fewer than three anchor-eligible rungs remain after dropping the target (page-quality scoping widens to the whole pool first — see above — and refuses this only if the whole pool is still that thin).
