...
A ladder-tier row whose exact bucket was scoped narrower reports it in the fields BUI-939 introduced (`page_quality_fallback: true`, `page_quality_fallback_reason: "ladder_starved"`) — now on every such row, not only the ones a rung count would have rescued. The name is historical: it means "this row's rungs are the whole pool's, because the same-quality pool was too thin at the exact grade to price from".
