...
def test_cli_add_with_locg_ids(monkeypatch):
    """--locg-id and --locg-variant-id flags are sent to server."""
    monkeypatch.setenv("GIXEN_SERVER_URL", "http://localhost:8080")
    monkeypatch.setenv("GIXEN_USERNAME", "u")
    monkeypatch.setenv("GIXEN_PASSWORD", "p")
    ...
    with runner.isolated_filesystem():
        with patch("cli.requests") as mock_req:
            ...
            result = runner.invoke(cli_app, [
                ...
                "--grade", "9.2",
                "--locg-id", "6977652",
                "--locg-variant-id", "6977652",
            ])
            assert result.exit_code == 0
            payload = mock_req.post.call_args[1]["json"]
            assert payload["locg_id"] == 6977652
            assert payload["locg_variant_id"] == 6977652


def test_cli_edit_with_locg_ids(monkeypatch):
    """`edit --locg-id N` sends locg_id in PATCH payload."""
    monkeypatch.setenv("GIXEN_SERVER_URL", "http://localhost:8080")
    ...
    with patch("cli.requests") as mock_req:
        ...
        mock_resp.raise_for_status.return_value = None
        mock_resp.json.return_value = {
            "item_id": "111222333", "max_bid": 800.0, "status": "PENDING",
        }
        mock_req.patch.return_value = mock_resp
        result = runner.invoke(cli_app, [
            "edit", "111222333", "800.00",
            "--locg-id", "6977652",
            "--locg-variant-id", "6977653",
        ])
        assert result.exit_code == 0
        payload = mock_req.patch.call_args[1]["json"]
        assert payload["locg_id"] == 6977652
        assert payload["locg_variant_id"] == 6977653
